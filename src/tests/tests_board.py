...
class BoardInitTests(unittest.TestCase):
    ...
    def test_boardinit4(self):
        board = Board(8, 10)
        self.assertEqual(board.squares[9][0], 'TW   ')
...
class CheckTests(unittest.TestCase):
    ...
    def test_correct_move(self):
        self.assertTrue(self.board.check_correct_move((1, 1), (2, 1))['output'])
        self.assertFalse(self.board.check_correct_move((1, 1), (2, 2))['output'])
        self.assertFalse(self.board.check_correct_move((1, 1), (1, 1))['output'])
    ...
    def test_obtain_other_turn(self):
        self.assertEqual(self.board.obtain_other_turn('W'), 'b')
        self.assertEqual(self.board.obtain_other_turn('B'), 'w')

    def test_check_is_pawn_first_movement(self):
        self.assertTrue(self.board.is_first_pawn_movement((1, 1)))

        self.board.put_pos_val((2, 1), 'PB   ')
        self.assertFalse(self.board.is_first_pawn_movement((2, 1)))
        
    def test_check_free_way(self):
        self.assertTrue(self.board.check_free_way([2, 0], (1, 0), (3, 0), 1)['output'])

        self.board.put_pos_val((2, 0), 'QW   ')
        self.assertFalse(self.board.check_free_way([2, 0], (1, 0), (3, 0), 1)['output'])
